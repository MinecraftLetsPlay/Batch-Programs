...
echo set WshShell = WScript.CreateObject("WScript.Shell") > %tmp%\CANCELLED.vbs
echo WScript.Quit (WshShell.Popup( "The operation was cancelled due to high risk of damaging the operating system" ,10 ,"OPERATION CANCELLED", vbCritical + 0)) >> %tmp%\CANCELLED.vbs
...
cls
echo set WshShell = WScript.CreateObject("WScript.Shell") > %tmp%\TK_Warn.vbs
echo WScript.Quit (WshShell.Popup( "The Process [ %TK% ] is a System Process! Killing such processes could damage the PC and will, most likely, result in a Bluescreen!" ,30 ,"KILL PROCESS?", vbExclamation + 4)) >> %tmp%\TK_Warn.vbs
cls
if %TK%==winlogon.exe (cscript /NOLOGO %tmp%\TK_Warn.vbs
goto POPUP_RETURN
)
if %TK%==wininit.exe (cscript /NOLOGO %tmp%\TK_Warn.vbs
goto POPUP_RETURN
)
if %TK%==smss.exe (cscript /NOLOGO %tmp%\TK_Warn.vbs
goto POPUP_RETURN
)
if %TK%==csrss.exe (cscript /NOLOGO %tmp%\TK_Warn.vbs
goto POPUP_RETURN
)
if %TK%==logonui.exe (cscript /NOLOGO %tmp%\TK_Warn.vbs
goto POPUP_RETURN
)
if %TK%==lsass.exe (cscript /NOLOGO %tmp%\TK_Warn.vbs
goto POPUP_RETURN
)
if %TK%==services.exe (cscript /NOLOGO %tmp%\TK_Warn.vbs
goto POPUP_RETURN
)
if %TK%==system.exe (cscript /NOLOGO %tmp%\TK_Warn.vbs
goto POPUP_RETURN
)
if %TK%==svchost.exe (cscript /NOLOGO %tmp%\TK_Warn.vbs
goto POPUP_RETURN
)
...
:POPUP_RETURN
if %errorlevel%==7 (del %tmp%\TK_Warn.vbs
cls
goto TK
)
if %errorlevel%==6 (del %tmp%\TK_Warn.vbs
cls
taskkill /f /im %TK%
echo.
pause
...
echo.
echo.
echo  WARNING: DELETING SYSTEM FILES CAN CAUSE SERIOUS DAMAGE TO THE PC! IT CAN CAUSE THE PC TO NOT BOOT CORRECTLY ANYMORE!!!
echo.
echo.
set /p F=TARGET(S):
cls

if %F%==%windir% (cscript /NOLOGO %tmp%\CANCELLED.vbs
exit
)
if %F%==C:\Windows\ (cscript /NOLOGO %tmp%\CANCELLED.vbs
exit
)
if %F%==C:\windows (cscript /NOLOGO %tmp%\CANCELLED.vbs
exit
...
if %F%==%windir%\System32 (cscript /NOLOGO %tmp%\CANCELLED.vbs
exit
)
if %F%==C:\Windows\System32\ (cscript /NOLOGO %tmp%\CANCELLED.vbs
exit
)
if %F%==C:\windows\System32 (cscript /NOLOGO %tmp%\CANCELLED.vbs
exit
